...
- **Terminal (PTU) in auth** — a cashier can only log in once MIN, SN and PTU_NO are configured in the helper's `config.json`; login reads them (`AuthProvider.loginUser`) and keeps them in `localStorage` (`utils/terminalSession.js`) until logout. Admin/manager skip this. `api/*` modules send the PTU automatically, so shifts (`cashier_reports.ptuNumber`), the transaction list, X-report and Z-report are scoped to the current terminal. Z-reports are per `(branch, ptuNumber, date)` with a per-PTU `Z_COUNTER`; admin views with no PTU list every terminal's report.
